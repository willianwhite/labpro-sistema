...
# Script para forçar atualização da API no KingHost
echo "=== FORÇAR ATUALIZAÇÃO DA API ==="
echo "Iniciado em: $(date)"
echo ""

# Configurações
WWW_DIR="/home/labpro/www"
API_FILE="$WWW_DIR/api/clientes_final.php"
CONFIG_FILE="$WWW_DIR/../config.php"

echo "Diretório WWW: $WWW_DIR"
echo "Arquivo API: $API_FILE"
...
if [ -f "$API_FILE" ]; then
    ...
    echo "Arquivo API não encontrado para backup"
fi

echo ""

# 2. Criar arquivo API corrigido
echo "=== 2. CRIANDO ARQUIVO API CORRIGIDO ==="
cat > "$API_FILE" << 'EOF'
<?php
...
echo "Arquivo API corrigido criado com sucesso!"
...
if [ -f "$API_FILE" ]; then
    echo "✅ Arquivo API existe"
    echo "Tamanho: $(stat -c%s %n "$API_FILE")"
    echo "Permissões: $(stat -c%s %n "$API_FILE")"
else
    echo "❌ Arquivo API não existe"
fi
...
echo ""
echo "API atualizada para usar configuração centralizada!"
echo "Teste em: http://labpro.web1f19.kinghost.net/api/clientes_final.php"
EOF

chmod +x force_update_api.sh
